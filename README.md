...
* Actions which merge two lines
  * And: this merges two lines together but it goes through every bit in each byte and it compares them to see if two bits at the same position in their respective bytes hold 
  a value of 1 and if they do, the bit from the bottom byte line will be carried into the merger with a positive value, otherwise it will be 
...
* Actions which affect only one line
  * Right shift and left sheft: moves every bit in the byte line by 1 cell in the direction chosen. A zero is added at the point of origin of the direction.
...
Actions are not free; you are limited to a certain number of actions by their type. For example, to solve one level, you could be limited to 4 shifts and 1 xor, so you'd
...
* Explosive: if it's sandwiched between two active bits on the vertical axis, those bits and itself become 0 except if the explosive bit is already 0.
* Virus: all adjacent bits become 1 when there's a change in the byte line except if the virus bit is 0.
* Wize: no matter what, it will was match the same value as the bit which holds the same position in the solution.
